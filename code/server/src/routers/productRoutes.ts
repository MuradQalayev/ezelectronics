...
class ProductRoutes {
	...
	initRoutes() {
		...
		this.router.get(
			...
			this.authenticator.isLoggedIn,
			this.authenticator.isAdminOrManager,
			query("grouping").optional().isString().notEmpty().isIn(["category", "model"]).custom((_value, { req }) => {
				if (["category", "model"].includes(req.query.grouping)) {
					// now we have two options, either it's set by category or by model
					if (req.query.grouping === "category" && req.query.category === undefined) {
						throw new Error("Invalid query parameters");
					} else if (req.query.grouping === "model" && req.query.model === undefined) {
						throw new Error("Invalid query parameters");
					} else {
						return true;
					}
			...
			query("category").optional().isString().notEmpty()
				.custom((_value, { req }) => {
					if (req.query.grouping === undefined || req.query.grouping === "model") {
						throw new Error("Invalid query parameters");
					} else if (req.query.grouping === "category" && ["Smartphone", "Laptop", "Appliance"].includes(req.query.category)) {
						return true;
					}
					throw new Error("Invalid query parameters");
				}),
			query("model").optional().isString().notEmpty()
				.custom((_value, { req }) => {
					if (req.query.grouping === undefined || req.query.grouping === "category") {
						throw new Error("Invalid query parameters");
					} else if (req.query.grouping === "model" && req.query.model.trim() !== "") {
						return true;
					}
					throw new Error("Invalid query parameters");
				}),
		...
		this.router.get(
			...
			(req: any, res: any, next: any) =>
				this.controller
					.getAvailableProducts(req.query.grouping, req.query.category, req.query.model)
					.then((products: Product[]) => res.status(200).json(products))
					.catch((err) => next(err)),
		);
